...
## Candidate ordering (已按透明度排序)
Candidates are pre-sorted by transparency so you can iterate from best to worst:
1. **transparent** (最优先): standard scene-release naming (e.g., "The.Dark.Knight.2008.2160p.BluRay.FGT")
2. **semi_transparent** (保底): non-standard but extractable metadata (e.g., "名称：莫离 4K臻彩MAX [HDR60fps][更新至36集]描述：...") — compliance-motivated long titles that still contain quality/episode info
3. **opaque** (最后): bare name with no metadata (e.g., "莫离", "【变形金刚系列】1~5部")

**保底候选处理**: semi_transparent candidates (合规长标题) appear in the middle. They contain useful info (片名、画质、剧集范围) despite verbose formatting. When transparent candidates don't cover the need, use these as fallback — the system has already extracted episodeHints/qualityHints from them.
...
## Candidate ordering (已按透明度排序)
Candidates are pre-sorted by transparency so you can iterate from best to worst:
1. **transparent** (最优先): standard naming (e.g., "The.Dark.Knight.2008.2160p.BluRay.FGT")
2. **semi_transparent** (保底): compliance-motivated long titles with extractable info (e.g., "名称：莫离 4K臻彩MAX [HDR60fps][更新至36集]描述：...")
3. **opaque** (最后): bare name with no metadata

**保底候选处理**: semi_transparent candidates contain useful info despite verbose formatting. Use them as fallback when transparent candidates don't cover the need.
